...
local Config = {} do
  ...
  -- Disabled: no external requests
  return false, "no-cached-link"
end
  ...
  -- Clipboard copy of cached link if available (no network)
  local fSetClipboard = setclipboard or toclipboard
  if not fSetClipboard then
    return false, "clipboard not available"
  end
  local ok, link = Config:CachLink()
  if ok then
  ...
  -- Local-only redeem: accept only hardcoded key
  if Val == "ikbaltamvan" then
    return true, "redeemed-local"
  end
  return false, "invalid-key-local"
end
  ...
  -- Local-only verification: accept only hardcoded key
  if not Val then return false, "no-key-provided" end
  if Val == "ikbaltamvan" then
    return true, "verified-local"
  end
  return false, "key-not-valid-local"
ending == true then
